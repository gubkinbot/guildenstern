...
class Bot_logic:
    ...
    current_queue = (
        []
    )  # [{'queue_id':0, 'tg_user_id':0, 'time_start':0, 'last_companion': 0}, ...]
    current_sessions = (
        []
    ...
    def __init__(self, db, handler) -> None:
            ...
            self.current_queue.append(
                {
                    ...
                    "time_start": v["time_start"].timestamp(),
        ...
        for v in current_sessions:
            ...
            self.current_sessions.append(
                {
                    "session_id": v["id"],
                    "tg_user_id_a": tg_user_id_a,
                    "tg_user_id_b": tg_user_id_b,
                    "time_start": v["time_start"].timestamp(),
                }
            )
    ...
    def handler_button(self, tg_user_id, data, message_id, call_id):
        ...
        if session_id:
            ...
            if data[1] == "impudence":
                ...
                self.db.Add_log(
                    tg_user_id, session_id, impudence_msg, time_send, "from_bot", 0
                )
                log_id = self.db.Get_log_id_session_id_and_time_send(
                    session_id, time_send
                )
            ...
            if data[1] == "is_bot":
                self.remove_button_bot_from_last_mgs(
                    tg_user_id, tg_user_id_companion, 1, message_id, call_id
                )
        ...
        if data[1] == "buying_replace_name":
            ...
            if added_points[user_id] >= self.BUYING_REPLACE_NAME:
                ...
                self.answer(
                    call_id, f"> -{self.BUYING_REPLACE_NAME} очков", cache_time=60
                )
                pseudonym = ""
                while True:
                ...
                self.delete(tg_user_id, message_id)
                self.send(
                    tg_user_id, f"> Ваш новый псевдоним:\n- {pseudonym}", clear=True
                )
    ...
    def queue_schedule(self):
        ...
        will_be_connection = (
            {}
        )  # {'tg_user_id_a': ['tg_user_id_b','tg_user_id_c'], ... }
            ...
            if waiting_time_a > self.MIN_WAITING_IN_QUEUE:
                for user_b in self.current_queue:
                    ...
                    if (
                        (id_a != id_b)
                        and (
                            user_a["last_companion"] != id_b
                            or (
                                waiting_time_a > self.COMPANION_TIME_OUT
                                and waiting_time_b > self.COMPANION_TIME_OUT
                            )
                        )
                        and (waiting_time_b > self.MIN_WAITING_IN_QUEUE)
                        and (not will_be_connection.get(id_b))
                    ):
        ...
        for user_in_queue in self.current_queue:
            ...
            if waiting_time > self.MAX_WAITING_IN_QUEUE:
                ...
                self.stop_queue(
                    ...
                    time_stemp,
                    "NULL",
                    "> Превышено время ожидания в очереди. /start",
                    clear=True,
                )

        for session in self.current_sessions:
            last_message_timestamp = self.db.Get_last_message_timestamp_from_session_id(
                session["session_id"]
            )
            ...
            if waiting_time > self.MAX_WAITING_IN_QUEUE:
                ...
                self.stop_session(
                    tg_user_id,
                    time_stemp,
                    f"Timeout({self.MAX_WAITING_IN_SESSIONS})",
                    "> Превышено время ожидания в сессии. /start",
                )
    ...
    def cmd_start(self, tg_user_id):
        ...
        if not self.db.Get_id_from_tg_user_id(tg_user_id):
            ...
            self.send(
                tg_user_id,
                f'''Добро пожаловать!\n\nВаш уникальный псевдоним:\n{pseudonym}\n\n👋 Нажми /start чтобы найти собеседника. Дальше все просто: общайся, знакомься, но делай это вежливо и интересно!

🤖 Иногда в ваш диалог встревает бот Антон. Если ты считаешь, что тебе пришло сообщение от бота, нажми кнопку 🤖

🎰 За каждый правильный ответ ты получишь 10 баллов.
За неправильный - ты и твой собеседник теряете по 5 баллов.
Если ничего не нажимать - баллы не изменяются.

🏆 Копи баллы и ищи себя в списке лидеров здесь - /top

🤬 Помни: здесь не любят агрессию. Если начнёшь ругаться, тебе придется иметь дело лично с Антоном!''',
            )
    ...
    def cmd_stop(self, tg_user_id):
        ...
        self.stop_queue(
            ...
            time_stemp,
            "NULL",
            "> Поиск остановлен. /start",
            clear=True,
        )
        self.stop_session(
            tg_user_id, time_stemp, "command_stop", "> Сессия остановлена. /start"
        )

    def cmd_info(self, tg_user_id):
        msg = f'''👋 Нажми /start чтобы найти собеседника. Дальше все просто: общайся, знакомься, но делай это вежливо и интересно!
...
🤬 Помни: здесь не любят агрессию. Если начнёшь ругаться, тебе придется иметь дело лично с Антоном!\n\n'''
    ...
    def cmd_top(self, tg_user_id):
        ...
        self.send(
            ...
            reply_markup=self.create_buttons(
                [f"🔄 Новый псевдоним - {self.BUYING_REPLACE_NAME} очков"],
                "buying_replace_name",
            ),
            clear=True,
        )
    ...
    def send_and_bot_button(
        ...
        msg = self.send(
            ...
            send_message,
            parse_mode="Markdown",
            reply_markup=self.create_buttons(["🤖"], "is_bot"),
        )

        self.is_bot.append(
            {
                "session_id": session_id,
                "tg_user_id": tg_user_id_companion,
                "message_id": msg.message_id,
                "text": send_message,
                "time_send": time_send,
            }
    ...
    def remove_button_bot_from_last_mgs(
        self, tg_user_id, tg_user_id_companion, grade=0, message_id=0, call_id=0
    ):
        for v in self.is_bot[:]:
            if (grade != 0 and v["message_id"] == message_id) or (
                ...
                self.edit(
                    chat_id=tg_user_id,
                    message_id=detected_bot["message_id"],
                    text=detected_bot["text"],
                )

                log_id = self.db.Get_log_id_session_id_and_time_send(
                    detected_bot["session_id"], detected_bot["time_send"]
                )
                ...
                if grade != 0:
                    ...
                    if self.db.Get_is_bot_from_log_id(log_id):
                        self.answer(
                            call_id,
                            f"это БОТ\n+{self.BONUS_SELECT_BOT_MESSAGE} очков",
                            cache_time=120,
                        )
                        self.db.Add_points(
                            tg_user_id,
                            self.BONUS_SELECT_BOT_MESSAGE,
                            detected_bot["time_send"],
                        )
                    else:
                        self.answer(
                            call_id,
                            f"это НЕ бот\n{self.BONUS_SELECT_NOT_BOT_MESSAGE} очков",
                            cache_time=120,
                        )
                        self.db.Add_points(
                            tg_user_id,
                            self.BONUS_SELECT_NOT_BOT_MESSAGE,
                            detected_bot["time_send"],
                        )
                        self.db.Add_points(
                            tg_user_id_companion,
                    ...
                    if log_id and self.db.Get_is_bot_from_log_id(log_id):
                        self.db.Add_points(
                            tg_user_id,
                            self.BONUS_NOT_SELECT_BOT_MESSAGE,
                            detected_bot["time_send"],
                        )
                    # else:
    ...
    def add_to_queue(self, tg_user_id, time_stemp):
        ...
        for sessions in self.current_sessions:
            if (
                sessions["tg_user_id_a"] == tg_user_id
                or sessions["tg_user_id_b"] == tg_user_id
            ):
        ...
        self.send(
            tg_user_id,
            f"{msg}\n> Поиск собеседника. Пожалуйста, подождите...",
            clear=True,
        )  # f'Please wait at least {self.MIN_WAITING_IN_QUEUE} seconds...')
    ...
    def stop_session(self, tg_user_id, time_stemp, status, bot_message):
        for session in self.current_sessions[:]:
            if (
                session["tg_user_id_a"] == tg_user_id
                or session["tg_user_id_b"] == tg_user_id
            ):
                self.db.Stop_session(session["session_id"], time_stemp, status)

                #
                self.remove_button_bot_from_last_mgs(
                    session["tg_user_id_a"], session["tg_user_id_b"]
                )
                self.remove_button_bot_from_last_mgs(
                    session["tg_user_id_b"], session["tg_user_id_a"]
                )

                added_points = self.db.Get_all_users_points_from_interval(
                    session["time_start"], time_stemp
                )
                ...
                added_points_a = (
                    added_points[user_id_a] if added_points.get(user_id_a) else 0
                )
                added_points_b = (
                    added_points[user_id_b] if added_points.get(user_id_b) else 0
                )
    ...
    def send(
        ...
        msg = self.bot_send(
            tg_user_id, message, parse_mode=parse_mode, reply_markup=reply_markup
        )
